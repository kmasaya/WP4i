...
class KtaiStyle_Config{
    ...
    public function icon_style(){
        global $Ktai_Style;
        ?>
        <link rel="stylesheet" type="text/css" id="ktai-style"
              href="<?php echo $Ktai_Style->get( 'plugin_url' ) . KtaiStyle::CONFIG_DIR; ?>/config.css"/>
    <?php
    }
        ...
        <div class="wrap">
            ...
            <form name="form" method="post" action="">
                ...
                <table class="form-table">
                    ...
                    <tr>
                        <th><label
                                for="allow_pictograms"><?php _e( 'Typing pictograms at comment forms, post contents', 'ktai_style' ); ?></label>
                        </th>
                        <td>
                            <label><input type="radio" name="ks_allow_pictograms" id="ks_allow_pictograms-0"
                                          value="0"<?php echo $allow_pictograms[0]; ?> /> <?php _e( 'Deny', 'ktai_style' ); ?>
                            </label><br/>
                            <label><input type="radio" name="ks_allow_pictograms" id="ks_allow_pictograms-1"
                                          value="1"<?php echo $allow_pictograms[1]; ?> /> <?php _e( 'Allow', 'ktai_style' ); ?>
                            </label>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="ks_separate_comments"><?php _e( 'Comments and Pings', 'ktai_style' ); ?></label>
                        </th>
                        <td>
                            <label><input type="radio" name="ks_separate_comments" id="ks_separate_comments-0"
                                          value="0"<?php echo $separate_comments[0]; ?> /> <?php _e( 'Mix them at each posts and/or recent comments.', 'ktai_style' ); ?>
                            </label><br/>
                            <label><input type="radio" name="ks_separate_comments" id="ks_separate_comments-1"
                                          value="1"<?php echo $separate_comments[1]; ?> /> <?php _e( 'Separate comments and pings.', 'ktai_style' ); ?>
                            </label>
                        </td>
                    </tr>
                    <tr>
                        <th><label
                                for="ks_require_term_id"><?php _e( 'Terminal ID of comment poster', 'ktai_style' ); ?></label>
                        </th>
                        <td>
                            <label><input type="radio" name="ks_require_term_id" id="ks_require_term_id-0"
                                          value="0"<?php echo $require_term_id[0]; ?> /> <?php _e( 'Not required', 'ktai_style' ); ?>
                            </label><br/>
                            <label><input type="radio" name="ks_require_term_id" id="ks_require_term_id-1"
                                          value="1"<?php echo $require_term_id[1]; ?> /> <?php _e( 'Required to send', 'ktai_style' ); ?>
                            </label><br/>
                    ...
                    <tr>
                        <th><label
                                for="ks_treat_as_internal"><?php _e( 'Websites to link directly', 'ktai_style' ); ?></label>
                        </th>
                        <td><textarea name="ks_treat_as_internal" id="ks_treat_as_internal" cols="60"
                                      rows="2"><?php echo $treat_as_internal; ?></textarea><br/><?php _e( '(Not using the redirect page for these sites; space separated)', 'ktai_style' ); ?>
                        </td>
                    </tr>
                    </tbody>
                </table>
                <p class="submit">
                    <input type="submit" name="update_option" class="button-primary"
    ...
    public function theme_page(){
        ...
        register_setting( self::THEME_OPTIONS, 'ks_theme_tablet' );
        register_setting( self::THEME_OPTIONS, 'ks_theme_smartphone' );
        ...
        $theme_tablet = $Ktai_Style->get_option( 'ks_theme_tablet' );
        $theme_smartphone = $Ktai_Style->get_option( 'ks_theme_smartphone' );
        ...
        <div class="wrap">
            ...
            <?php if( $theme_total ){
                ...
                if( $page_links ) : ?>
                    <div class="tablenav">
                        <div
                            class="tablenav-pages"><?php $page_links_text = sprintf( '<span class="displaying-num">' . __( 'Displaying %s&#8211;%s of %s', 'ktai_style' ) . '</span>%s', number_format_i18n( $start + 1 ), number_format_i18n( min( $page * self::THEMES_PER_PAGE, $theme_total ) ), number_format_i18n( $theme_total ), $page_links );
                                    ...
                                    <?php if( !empty( $theme_name ) ) :
                                        ...
                                        if( $screenshot ){
                                            ...
                                            <a href="<?php echo $preview_link; ?>"
                                               class="<?php echo $thickbox_class; ?> screenshot"
                                               style="width:180px;height:450px;"><img src="<?php echo $screenshot; ?>"
                                                                                      alt=""
                                                                                      style="width:180px;height:450px;"/></a>
                                        <?php } ?>
                                        <h3><?php
            ...
            <form name="form" method="post"
                  action="<?php echo sprintf( 'admin.php?page=%s', self::THEME_OPTIONS ) . ( $page > 1 ? ( '&pagenum=' . $page ) : '' ); ?>">
                <input type="hidden" name="action" value="update"/>
                <?php wp_nonce_field( self::THEME_OPTIONS ); ?>
                <table class="form-table">
                    <tbody>
                    <?php
                    $careers[] = array( 'label' => 'theme_tablet', 'desc' => __( 'For iPad, Android Tablet', 'ktai_style' ) );
                    $careers[] = array( 'label' => 'theme_smartphone', 'desc' => __( 'For iPhone, Android', 'ktai_style' ) );
                    ...
                    foreach( $careers as $index => $c ){
                        ?>
                        <tr>
                            <th><label for="ks_<?php echo $c['label']; ?>"><?php echo $c['desc']; ?></label></th>
                            <td><select name="ks_<?php echo $c['label']; ?>" id="ks_<?php echo $c['label']; ?>">
                                    <?php
                                    if( strcmp( $c['label'], 'theme_tablet' ) === 0 ){
                                    ...
                                    } elseif( strcmp( $c['label'], 'theme_smartphone' ) === 0 ){
                                        $current = empty( ${$c['label']} ) ? ' selected="selected"' : '';
                                        echo '<option value="0"' . $current . '>' . __( "(Don't use Ktai Style)", 'ktai_style' ) . '</option>';
                                        ?>
                                        <option value="<?php echo KtaiThemes::SAME_THEME_AS_COMMON; ?>"<?php selected( ${$c['label']}, KtaiThemes::SAME_THEME_AS_COMMON ); ?>><?php _e( '(Same as common theme)', 'ktai_style' ); ?></option><?php
                    ...
                    <?php } // foreach ?>
                    <tr>
                        <td colspan="2"><?php _e( 'Other phones (Windows Mobile, Palm, PSP, Nintendo DS, etc) are not able to select an exclusive theme.', 'ktai_style' ); ?></td>
                    </tr>
                    </tbody>
                </table>
                <p class="submit">
        ...
        <div class="wrap">
            ...
            <form name="form" method="post" action="">
                ...
                <table class="form-table">
                    <thead>
                    <tr>
                        <th><?php _e( 'Target', 'ktai_style' ); ?></th>
                        <td><?php _e( '#rrggbb as hex format', 'ktai_style' ); ?></td>
                        <td><?php _e( 'Color sample', 'ktai_style' ); ?></td>
                    </tr>
                    </thead>
                    <tbody>
                    <tr>
                        <th><label for="ks_date_color"><?php _e( 'Date/time for post titles', 'ktai_style' ); ?></label>
                        </th>
                        <td><input type="text" value="<?php echo esc_attr( $date_color ); ?>" name="ks_date_color"
                                   id="ks_date_color" onkeyup='change_sample(this, "date_color_sample")'/></td>
                        <td id="date_color_sample"
                            style="color:<?php echo esc_attr( $date_color ); ?>;"><?php echo date( 'Y-m-d H:i' ); ?></td>
                    </tr>
                    <tr>
                        <th><label
                                for="ks_author_color"><?php _e( 'Author, Date with a post content', 'ktai_style' ); ?></label>
                        </th>
                        <td><input type="text" value="<?php echo esc_attr( $author_color ); ?>" name="ks_author_color"
                                   id="ks_author_color" onkeyup='change_sample(this, "author_color_sample")'/></td>
                        <td id="author_color_sample"
                            style="color:<?php echo esc_attr( $author_color ); ?>;"><?php echo esc_html( $user_identity ); ?></td>
                    </tr>
                    <tr>
                        <th><label for="ks_comment_type_color"><?php _e( 'Comment types', 'ktai_style' ); ?></label>
                        </th>
                        <td><input type="text" value="<?php echo esc_attr( $comment_type_color ); ?>"
                                   name="ks_comment_type_color" id="ks_comment_type_color"
                                   onkeyup='change_sample(this, "comment_type_color_sample")'/></td>
                        <td id="comment_type_color_sample"
                            style="color:<?php echo esc_attr( $comment_type_color ); ?>;"><?php echo __( 'Comment', 'ktai_style' ), '/', __( 'Trackback' ), '/', __( 'Pingback' ); ?></td>
                    </tr>
                    <tr>
                        <th><label
                                for="ks_external_link_color"><?php _e( 'Link text for PC targeted external sites', 'ktai_style' ); ?></label>
                        </th>
                        <td><input type="text" value="<?php echo esc_attr( $external_link_color ); ?>"
                                   name="ks_external_link_color" id="ks_external_link_color"
                                   onkeyup='change_sample(this, "external_link_color_sample")'/></td>
                        <td id="external_link_color_sample"
                            style="color:<?php echo esc_attr( $external_link_color ); ?>;"><?php echo self::LINK_COLOR_URL_EXAMPLE; ?></td>
                    </tr>
                    <tr>
                        <th><label
                                for="ks_edit_color"><?php _e( 'Link of edit posts/pages/commetns for login user', 'ktai_style' ); ?></label>
                        </th>
                        <td><input type="text" value="<?php echo esc_attr( $edit_color ); ?>" name="ks_edit_color"
                                   id="ks_edit_color" onkeyup='change_sample(this, "edit_color_sample")'/></td>
                        <td id="edit_color_sample"
                            style="color:<?php echo esc_attr( $edit_color ); ?>;"><?php _e( 'Edit This' ); ?></td>
                    </tr>
                    </tbody>
                </table>
                ...
                <table class="form-table">
                    ...
                    <tr>
                        <th><label
                                for="ks_year_format"><?php _e( 'Datetime for last year and before<br />(year, month, and date required)', 'ktai_style' ); ?></label>
                        </th>
                        <td><input type="text" value="<?php echo $year_format ?>" name="ks_year_format"
                                   id="year_format"/></td>
                    </tr>
                    <tr>
                        <th><label
                                for="ks_month_date_format"><?php _e( 'Datetime for this year<br />(month and date required)', 'ktai_style' ); ?></label>
                        </th>
                        <td><input type="text" value="<?php echo $month_date_format ?>" name="ks_month_date_format"
                                   id="ks_month_date_format"/></td>
                    </tr>
                    <tr>
                        <th><label
                                for="ks_time_format"><?php _e( 'Datetime for today<br />(date or hour required)', 'ktai_style' ); ?></label>
                        </th>
                        <td><input type="text" value="<?php echo $time_format ?>" name="ks_time_format"
                                   id="ks_time_format"/></td>
                    </tr>
                    </tbody>
                </table>
    ...
    public function delete_page(){
        ...
        register_setting( self::DELETE_OPTIONS, 'ks_theme_tablet' );
        register_setting( self::DELETE_OPTIONS, 'ks_theme_smartphone' );
        ...
        <div class="wrap">
            ...
            <form name="form" method="post" action="">
                ...
                <p class="submit">
                    <input type="submit" name="delete_option"
                           value="<?php _e( 'Delete option values and revert them to default &raquo;', 'ktai_style' ); ?>"
                           onclick="return confirm('<?php _e( 'Do you really delete option values and revert them to default?', 'ktai_style' ); ?>')"/>
                </p>
            </form>
        </div>
    <?php
    ...
    public function delete_options(){
        $theme_opts = array( 'ks_theme', 'ks_theme_tablet', 'ks_theme_smartphone', 'ks_theme_foma', 'ks_theme_ezweb', 'ks_theme_sb_3g', 'ks_theme_willcom', 'ks_theme_emobile' );
        foreach( $theme_opts as $t ){
            delete_option( $t );
        }
