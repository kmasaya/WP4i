...
function _ks_internal_link( $link, $accesskey, $label, $post_password = NULL, $before = '', $after = '' ){
    if( $post_password && !ks_post_password_required() ){
        ...
        $output = '<form method="post" action="' . esc_attr( $link ) . '">' . $param . '<input type="hidden" name="post_password" value="' . htmlspecialchars( $post_password, ENT_QUOTES ) . '" />' . $before . '<label' . ks_accesskey_html( $accesskey ) . '><input type="submit" name="submit" value="' . esc_attr( $label ) . '" /></label>' . $after . '</form>';
    } else{
...
function ks_fix_encoding_form( $echo = TRUE ){
    $output = '<input type="hidden" name="charset_detect" value="' . __( 'Encoding discriminant strings to avoid charset mis-understanding', 'ktai_style' ) . '" />';
    if( $echo ){
        echo $output;
    }
...
function ks_page_title(){
    $defaults = array( 'logo_file' => '', 'echo' => TRUE, 'before' => '<h1>', 'after' => '</h1>', 'before_logo' => '<h1 align="center">', );
    $r = _ks_parse_arg( func_get_args(), $defaults );
...
function ks_content(){
    global $id;
    $defaults = array( 'more_link_text' => '(more...)', 'stripteaser' => 0, 'more_file' => '', 'strip_length' => 0, 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
    ...
    if( $r['strip_length'] && strlen( $content ) > $r['strip_length'] ){
        ...
        $content .= ( empty( $r['more_link_text'] ) ? apply_filters( 'excerpt_more', '[...]' ) : sprintf( '<span><a href="%1$s#more-%2$d">%3$s</a></span>', get_permalink(), $id, $r['more_link_text'] ) );
...
function ks_link_pages(){
    ...
    $defaults = array( 'before' => '<p>' . __( 'Pages:' ), 'after' => '</p>', 'next_or_number' => 'number', 'nextpagelink' => __( 'Next page' ), 'previouspagelink' => __( 'Previous page' ), 'pagelink' => '%', 'more_file' => '', 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
    extract( $r, EXTR_SKIP );
 ...
 */
function ks_pagenum(){
    $defaults = array( 'before' => ' (', 'after' => ')', 'echo' => TRUE, 'paged' => 0, 'max_pages' => 0, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
...
function ks_previous_post_link(){
    $defaults = array( 'format' => '<img localsrc="7" alt="&laquo; ">*.%link', 'link' => '%title', 'in_same_cat' => FALSE, 'excluded_categories' => '', 'accesskey' => '*', 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
...
function ks_next_post_link(){
    $defaults = array( 'format' => '#.%link<img localsrc="8" alt=" &raquo;">', 'link' => '%title', 'in_same_cat' => FALSE, 'excluded_categories' => '', 'accesskey' => '#', 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
...
function ks_posts_nav_link(){
    ...
    $defaults = array( 'sep' => ' | ', 'before' => '', 'after' => '', 'prev_anchor' => NULL, 'next_anchor' => NULL, 'prev_key' => '*', 'next_key' => '#', 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
...
function ks_posts_nav_multi(){
    $defaults = array( 'num' => 3, 'first' => __( 'First', 'ktai_style' ), 'last' => __( 'Last', 'ktai_style' ), 'prev_key' => '*', 'next_key' => '#', 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
...
function ks_posts_nav_dropdown(){
    ...
    $defaults = array( 'before' => '', 'after' => '', 'min_pages' => 2, 'show_all_limit' => 13, 'paged' => 0, 'max_pages' => 0, 'id' => 'paged', 'baseurl' => NULL, 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
    ...
    if( $r['max_pages'] <= $r['show_all_limit'] ){
        ...
        for( $digit = 1, $has_lower = $has_upper = TRUE; $has_lower || $has_upper; $digit *= 10 ){
            while( $has_lower ){
                if( $r['paged'] - $digit <= 1 ){
                    $has_lower = FALSE;
                    break;
...
function ks_back_to_post(){
    $defaults = array( 'icon' => '<img localsrc="64" alt="' . __( '&lt;-', 'ktai_style' ) . '" />', 'anchor' => __( 'Back to the post', 'ktai_style' ), 'accesskey' => '*', 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
    if( isset( $r['label'] ) ){
        $r['anchor'] = $r['label'];
    }
...
function ks_get_recent_comments(){
    $defaults = array( 'num' => 20, 'type' => '', 'group_by_post' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
...
function ks_category(){
    $defaults = array( 'separator' => ', ', 'parents' => '', 'color' => '', 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
...
function ks_dropdown_categories(){
    global $Ktai_Style;
    $defaults = array( 'show_count' => 1, 'show_option_all' => __( 'All', 'ktai_style' ), 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
 ...
 */
function ks_tags(){
    $defaults = array( 'before' => '', 'after' => '', 'separator' => ', ', 'color' => '', 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
...
function ks_tag_cloud( $args = '' ){
    $defaults = array( 'number' => 45, 'format' => 'flat', 'orderby' => 'name', 'order' => 'ASC', 'exclude' => '', 'include' => '', 'color' => '', 'echo' => TRUE, );
...
function _ks_generate_tag_cloud( $tags, $args = '' ){
    ...
    $defaults = array( 'smallest' => 1, 'largest' => 6, 'unit' => '', 'number' => 45, 'format' => 'flat', 'orderby' => 'name', 'order' => 'ASC', 'color' => '', );
    $r = wp_parse_args( $args, $defaults );
    extract( $r );
...
function ks_switch_pc_view(){
    global $Ktai_Style;
    $defaults = array( 'before' => ' (', 'after' => ')', 'color' => '', 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
...
function ks_switch_inline_images(){
    global $Ktai_Style;
    $defaults = array( 'before' => '<hr /><div align="center">', 'after' => '</div>', 'color' => '', 'echo' => TRUE, );
    $r = _ks_parse_arg( func_get_args(), $defaults );
   ...
   ================================================== */
    class KS_Walker_Comment extends Walker{
        ...
        public function start_el( &$output, $comment, $depth, $args ){
            ...
            ?><img localsrc="46" alt=" @ "/><font
                color="<?php echo ks_option( 'ks_date_color' ); ?>"><?php ks_comment_datetime(); ?></font>
