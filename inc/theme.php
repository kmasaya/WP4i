...
class KtaiThemes{
    ...
    private $theme_data;
    public static $target = array( 'tablet', 'smartphone', 'foma', 'ezweb', 'sb_3g', 'willcom', 'emobile' );
    ...
    public static function valid_dir_name( $dir ){
        $valid = FALSE;
        if( !empty( $dir ) && $dir !== self::SAME_THEME_AS_COMMON && !preg_match( '![^-_/a-zA-Z0-9]!', $dir ) && !validate_file( $dir ) ){
            $valid = TRUE;
        }
    ...
    public static function valid_theme( $path ){
        ...
        if( is_dir( $path ) ){
            ...
            if( file_exists( $path . 'index.php' ) && file_exists( $path . 'style.css' ) ){
                $valid = TRUE;
            }
        }
    ...
    public static function set_theme( $dir, $target = FALSE ){
        ...
        } elseif( $target == 'tablet' && $dir == self::SAME_THEME_AS_COMMON ){
            update_option( self::OPTION_PREFIX . '_tablet', self::SAME_THEME_AS_COMMON );
    ...
    public function get_theme_info( $dir ){
        ...
        if( !self::valid_dir_name( $this->theme ) || !self::valid_theme( $this->template_dir ) ){ // fall back to the default theme
    ...
    private function get_attachment_template(){
        ...
        if( $template = $this->query_template( $type[0] ) )
            return $template; elseif( $template = $this->query_template( $type[1] ) )
            return $template; elseif( $template = $this->query_template( "$type[0]_$type[1]" ) )
...
function ks_theme_url( $echo = TRUE ){
    ...
    $theme_root = $Ktai_Style->theme->get( 'theme_root' ); // with trailing slash
    $theme_url = apply_filters( 'ktai_theme_url', $Ktai_Style->strip_host( $theme_url ), $theme, $theme_root );
    if( $echo ){
        echo $theme_url;
    }
