            ...
            <div>
                ...
                <?php if( 'draft' != $post->post_status ){ ?>
                    <input type="hidden" name="_wp_original_http_referer"
                           value="<?php echo esc_attr( $this->orig_referer ); ?>"/>
                <?php
                }
                echo $form_extra;
                _e( 'Title' ); ?><br/>
                <input type="text" name="post_title" size="32" maxlength="999" tabindex="1"
                       value="<?php echo $post->post_title; ?>"/><br/>
                <?php if( $can_publish ){
                    _e( 'Slug' );
                    echo esc_html( $slug_optional ); ?><br/>
                    <input type="text" name="post_name" size="24" maxlength="999" istyle="3" mode="alphabet"
                           tabindex="2" value="<?php echo $post->post_name; ?>"/><br/>
                <?php
                }
                _e( 'Categories' ); ?><br/>
                <input type="hidden" name="post_cats" value="<?php echo implode( ',', $checked_cats ); ?>"/><font
                    color="green"><?php echo implode( ', ', $cat_names ); ?></font><input type="submit" name="selcats"
                                                                                          tabindex="3"
                                                                                          value="<?php _e( 'Change', 'ktai_style' ); ?>"/><br/>
                <?php _e( 'Content', 'ktai_style' ); ?><br/>
                <?php $this->admin->the_editor( $post->post_content );
                _e( 'Tags' ); ?>
                <br/><input type="text" name="tags_input" size="48" tabindex="5"
                            value="<?php echo( isset( $post->tags_input ) ? $post->tags_input : get_tags_to_edit( $post->ID ) ); ?>"/><br/>

                <div><label><input type="checkbox" name="comment_status" tabindex="6"
                                   value="open" <?php checked( $post->comment_status, 'open' ); ?> /><?php _e( 'Allow Comments' ); ?>
                    </label><br/>
                    <label><input type="checkbox" name="ping_status" tabindex="7"
                                  value="open" <?php checked( $post->ping_status, 'open' ); ?> /> <?php _e( 'Allow Pings' ); ?>
                    </label></div>
                <?php
                ...
                if( !in_array( $post->post_status, array( 'publish', 'future', 'private' ) ) || 0 == $post->ID ){
                    if( $can_publish ){
                        ...
                        ?><input type="submit" name="publish" tabindex="p"
                                 value="<?php _e( 'Submit for Review' ) ?>" /><?php
                    }
                }
                if( 'edit' == $action && current_user_can( 'delete_post', $post->ID ) ){
                    ...
                    ?><br/><img localsrc="61"/><a href="<?php echo esc_attr( $delete_url ); ?>"><font
                        color="fuchsia"><?php echo $delete_text; ?></font></a><?php
                } ?>
                <hr/>
                <?php
                    ...
                    <select name="post_status">
                        ...
                        <?php elseif( 'private' == $post->post_status ) : ?>
                            <option<?php selected( $post->post_status, 'private' ); ?>
                                value='publish'><?php _e( 'Privately Published', 'ktai_style' ) ?></option>
                ...
                if( $can_publish ){
                    ?><label><input type="checkbox" name="visibility" tabindex="10"
                                    value="private" <?php checked( $visibility, 'private' ); ?> /><?php _e( 'Turn to private', 'ktai_style' ); ?>
                    </label><br/><?php
                }
                if( $post_ID ){
                    echo '<br />';
