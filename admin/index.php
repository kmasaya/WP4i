...
class KtaiAdmin_Dashboard{
    ...
    public function recent_drafts( $drafts = FALSE ){
        ...
        <?php
        if( !$drafts ){
            $drafts_query = new WP_Query( array( 'post_type' => 'post', 'what_to_show' => 'posts', 'post_status' => 'draft', 'author' => $GLOBALS['current_user']->ID, 'posts_per_page' => 5, 'orderby' => 'modified', 'order' => 'DESC' ) );
        ...
        if( $drafts && is_array( $drafts ) ){
            ...
            <div align="right"><img localsrc="63" alt="&#8594;"/><a
                    href="edit.php?post_status=draft"><?php _e( 'View all', 'ktai_style' ); ?></a></div>
        <?php
        } else{
            ?>
    ...
    public function recent_comments(){
        ...
        if( $comments ){
            ...
            <?php if( current_user_can( 'edit_posts' ) ){ ?>
                <div align="right"><img localsrc="63" alt="&#8594;"/><a
                        href="edit-comments.php"><?php _e( 'View all', 'ktai_style' ); ?></a></div>
            <?php
            }
        } else{
            ?>
    ...
    public function recent_comments_row( &$comment ){
        ...
        if( !$comment->comment_type || 'comment' == $comment->comment_type ) :
            $desc = sprintf( __( 'From %s on %%link', 'ktai_style' ), get_comment_author() ); else :
        ...
        if( current_user_can( 'edit_post', $comment->comment_post_ID ) ){
            ?>
            <dd>[ <img localsrc="149" alt=""/><a
                    href="<?php echo esc_attr( $reply_url ); ?>"><?php _e( 'Reply', 'ktai_style' ); ?></a>
                | <img localsrc="104" alt=""/><a href="<?php echo esc_attr( $edit_url ); ?>"><?php _e( 'Edit' ); ?></a>
                ]
            </dd>
        <?php
        }
