...
class KtaiAdmin_Edit_Posts{
    ...
    private function edit_posts_query(){
        ...
        $query = array( 'post_type' => 'post', 'what_to_show' => 'posts', 'post_status' => 'any', 'posts_per_page' => $per_page, 'paged' => $pagenum, 'year' => $this->year, 'monthnum' => $this->monthnum, 'cat' => $this->cat, 'tag' => $this->tag, 'author' => intval( $_GET['author'] ), 's' => $this->search, );
    ...
    public function status_menu( $post_stati ){
        ...
        $submenu[$parent_file] = array( array( __( 'Add New Post' ), 'edit_posts', 'post-new.php' ), array( __( 'Any', 'ktai_style' ), 'edit_posts', $link ), );
    ...
    public function output(){
        ...
        <form action="" method="get">
            <div><?php
                ...
                _e( 'Search' ); ?><input type="text" name="ks" value="<?php echo wp_specialchars( $this->search ); ?>"
                                         size="17"/><br/>
                <?php
                ...
                <?php if( $this->is_trash && current_user_can( 'edit_others_posts' ) ){
                    ...
                    <br/><input type="submit" name="delete_all" id="delete_all"
                                value="<?php esc_attr_e( 'Empty Trash' ); ?>"/>
                <?php } ?>
            </div>
        </form>
        <?php if( have_posts() ){
