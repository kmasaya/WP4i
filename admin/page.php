...
class KtaiAdmin_Pages{
    ...
    public function redirect_page( $page_ID ){
        ...
        if( ( isset( $_POST['save'] ) || isset( $_POST['publish'] ) ) && ( empty( $referredby ) || $referredby == $referer || 'redo' != $referredby ) ){
            ...
            if( strpos( $orig_referer, $admin_dir . '/page.php' ) === FALSE && strpos( $orig_referer, $admin_dir . '/page-new.php' ) === FALSE ){
                $location = add_query_arg( array( '_wp_original_http_referer' => urlencode( $orig_referer ), 'message' => 1 ), get_edit_post_link( $page_ID, 'url' ) );
            } else{
