...
class KtaiAdmin_Edit_Comments{
    ...
    private function comments_stati( $num_comments, $comment_status, $comment_type ){
        $stati = array( 'all' => _n_noop( 'All', 'All' ), // singular not used
            'moderated' => _n_noop( 'Pending (%s)', 'Pending (%s)' ), 'approved' => _n_noop( 'Approved', 'Approved' ), // singular not used
        ...
        foreach( $stati as $status => $label ){
            ...
            $submenu[$parent_file][] = array( sprintf( _n( $label[0], $label[1], $num_comments->$status, 'ktai_style' ), number_format_i18n( $num_comments->$status ) ), 'edit_posts', $link );
    ...
    private function comment_list_item( $_comment ){
        ...
        <img localsrc="46" alt=" @ "/><font
            color="<?php echo ks_option( 'ks_date_color' ); ?>"><?php echo ks_comment_datetime(); ?></font>
        <img localsrc="<?php comment_type( 68, 112, 112 ); ?>"
        ...
        if( $comment->comment_author_url && 'http://' != $comment->comment_author_url ){
            ...
            if( strlen( $author_url_display ) > self::COMMENT_AUTHOR_URL_LENGTH )
                $author_url_display = substr( $author_url_display, 0, self::COMMENT_AUTHOR_URL_LENGTH - 1 ) . '...';
            ?><img localsrc="112" alt=""/><font color="olive"><?php echo esc_html( $author_url_display ); ?></font>
            <br/><?php
        }
