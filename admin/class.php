...
class KtaiStyle_Admin{
    ...
    public function set_session( $user_id, $sid = NULL, $data = NULL, $remember = FALSE ){
        ...
        $result = $wpdb->insert( $wpdb->prefix . 'ktaisession', array( 'sid' => $sid, 'expires' => date( 'Y-m-d H:i:s', $expires ), 'user_id' => $user_id, 'user_agent' => $ua_hash, 'term_id' => $tid_hash, 'sub_id' => $sub_hash, 'data' => ( $data ? serialize( $data ) : '' ), ) );
        if( !$result ){
            return NULL;
        }
        $this->sid = $sid;
    ...
    public function add_sid( $uri, $display = FALSE ){
        if( $this->sid ){
            ...
            $uri .= sprintf( '%s=%s', self::SESSION_NAME, $this->sid );
        }
