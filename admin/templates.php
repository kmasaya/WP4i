...
class KtaiStyle_AdminTemplates extends KtaiStyle_Admin{
    ...
    public function get_post_states( $post ){
        ...
        if( isset( $_GET['post_status'] ) )
            $post_status = $_GET['post_status']; else
            $post_status = '';
    ...
    public function nav_link( $current, $total, $query = 'paged' ){
        ...
        $nav = '';
        if( $current > 1 ){
            $nav .= '<a href="' . add_query_arg( $query, $current - 1, $link ) . '" accesskey="*">' . __( '<img localsrc="7" alt="&laquo;" />Previous Page', 'ktai_style' ) . '</a>';
        }
        if( $current < $total ){
            if( $nav ){
                $nav .= ' | ';
            }
            $nav .= '<a href="' . add_query_arg( $query, $current + 1, $link ) . '" accesskey="#">' . __( 'Next Page<img localsrc="8" alt="&raquo;" />', 'ktai_style' ) . '</a>';
        }
        if( $nav ){
    ...
    public function nav_dropdown( $current, $total, $query = 'paged' ){
        if( $total == 2 ){
            ...
            ks_posts_nav_dropdown( array( 'before' => '<div align="center">', 'after' => '<br /></div>', 'min_pages' => 3, 'show_all_limit' => 9, 'paged' => $current, 'max_pages' => $total, 'id' => $query, 'baseurl' => remove_query_arg( array( $query, 'filter', 'action', 'wpnonce' ) ), ) );
        }
    }
    // ===== End of class ====================
}
...
class KtaiCategory_Checklist extends Walker{
    ...
    function start_el( &$output, $category, $depth, $args ){
        ...
        $output .= '<dt>' . '<label><input value="' . $category->cat_ID . '" type="checkbox" name="cat[]" ' . ( in_array( $category->cat_ID, $selected_cats ) ? ' checked="checked"' : "" ) . '/> ' . esc_html( apply_filters( 'the_category', $category->cat_name ) ) . '</label>';
