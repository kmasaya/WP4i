...
<?php if( have_posts() ) : ?>
    ...
    <?php for( $count = 0; have_posts(); $count++ ) : the_post(); ?>
        ...
        color="<?php echo $ks_settings['date_color']; ?>"><?php the_time( __( 'Y.m.d H:i', 'redportal' ) ); ?></font><?php
        edit_post_link( '<font color="' . $ks_settings['edit_color'] . '">' . __( 'Edit' ) . '</font>', ' <img localsrc="104" alt="" />' );
        ?><br/>
    <?php endfor;
    if( $wp_query->max_num_pages >= 2 ) : ?>
        <div align="right"><br/><?php _e( '&rarr; ', 'redportal' );
            ks_next_posts_link( __( 'Read More', 'redportal' ) ); ?></div>
    <?php endif; else: ?>
