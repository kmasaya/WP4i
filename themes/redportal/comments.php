...
function ks_comment_content(){
    ...
    ?><img localsrc="46" alt=" @ "/><font
        color="<?php echo $ks_settings['date_color']; ?>"><?php ks_comment_datetime(); ?></font>
    <?php edit_comment_link( '<font color="' . $ks_settings['edit_color'] . '">' . __( 'Edit' ) . '</font>', '<img localsrc="104" alt="" />' ); ?>
    <br/>
...
function ks_list_comments( $comment, $args, $depth ){
    ...
    comment_reply_link( array_merge( array( 'before' => '<div>', 'after' => '</div><br />', 'reply_text' => '<img localsrc="149" alt="" />' . __( 'Reply' ), 'login_text' => '<img localsrc="120" alt="" />' . __( 'Log in to Reply' ) ), $args, array( 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
}
...
if( ks_is_comments_list() ) :
    ...
    foreach( $sep_comments as $type => $_comments ) :
        ...
        <?php else : // If there are no comments yet ?>
            <p><?php echo ( $type != 'pings' ) ? __( 'No comments yet.' ) : __( 'No pings yet.', 'ktai_style' );
                ?></p>
        <?php endif;
    endforeach;
    ks_comments_post_link( __( 'Write a comment', 'redportal' ), '<hr color="' . $ks_settings['hr_color'] . '" />', '', ks_pict_number( 1 ), '1' ); elseif( ks_is_comment_post() ) : ?>
    ...
    <?php
        ...
        if( $replytocom ){
            ...
            ks_comment_content();
            echo '<hr color="' . $ks_settings['hr_color'] . '" /><p>' . sprintf( __( 'Reply to comment #%1$d of <a href="%2$s">%3$s</a> of &quot;%4$s&quot;', 'redportal' ), $replytocom, get_permalink(), get_the_title(), get_bloginfo( 'name' ) ) . '</p>';
        } else{
        ...
        if( is_user_logged_in() ){
            ...
            ?> [<a
                href="<?php echo esc_url( ks_get_logout_url( KTAI_NOT_ECHO, ks_comments_post_url() ) ); ?>"><?php _e( 'Log out' ); ?></a>
            ]<br/><?php
            if( !ks_cookie_available() ){
                ?>
                <small><?php _e( 'Note: Ater posting a comment, you are automatically logged out.', 'ktai_style' ); ?></small>
                <br/><?php
            }
            ...
            <textarea name="comment" cols="100%" rows="4"><?php echo esc_html( $comment_content ); ?></textarea>
            <br/><?php
        } else{
            if( $login_url ){
                ?>
                <div align="right"><img localsrc="120"
                                        alt=""/><?php printf( __( '<a href="%s">Log in</a> and post a comment.', 'ktai_style' ), esc_url( $login_url ) ); ?>
                </div><?php
            }
            ?><p><font
                color="orange"><?php _e( '&diams;', 'redportal' ); ?></font><?php _e( 'Nickname', 'redportal' ); ?><?php if( $req )
                _e( '(required)' ); ?><br/>
            ...
            <font
                color="orange"><?php _e( '&diams;', 'redportal' ); ?></font><?php _e( 'Mail (will not be published)', 'ktai_style' );
            if( $req )
                _e( '(required)' ); ?><br/>
            <input type="text" name="email" istyle="3" mode="alphabet"
                   value="<?php echo esc_attr( $comment_author_email ); ?>"/><br/>
            <font color="orange"><?php _e( '&diams;', 'redportal' ); ?></font><?php _e( 'URL', 'redportal' ); ?><br/>
            <input type="text" name="url" istyle="3" mode="alphabet"
                   value="<?php echo esc_attr( $comment_author_url ); ?>"/><br/>
        <?php
        } // is_user_logged_in()
        ...
        <?php if( ks_is_required_term_id() ){ ?>
            <div><font
                    color="orange"><?php _e( '&diams;', 'redportal' ); ?></font><?php _e( 'NOTE: If submit comments, your terminal ID will be sent.', 'ktai_style' ); ?>
            </div>
        <?php
        }
        if( !is_user_logged_in() && ks_cookie_available() ){
            ?>
            <div><font
                    color="orange"><?php _e( '&diams;', 'redportal' ); ?></font><?php _e( 'NOTE: Your name, e-mail and URL will be stored o your phone. (If cookie is ON)', 'ktai_style' ); ?>
            </div>
        <?php
        }
