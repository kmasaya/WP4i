...
<?php if( have_posts() ) : the_post();
    ...
    <div align="right"><font size="-1"><a href="#cont" accesskey="<?php echo $next_accesskey; ?>"><img localsrc="30"
                                                                                                       alt="<?php _e( '&darr;', 'ktai_style' ); ?>"/><?php _e( 'Skip the content', 'ktai_style' ); ?>
            </a><?php ks_pict_number( $next_accesskey, TRUE ); ?></font></div>
    <h2><?php if( ks_is_front() ){ ?>
            <img localsrc="334" alt="[new] "/>
        <?php
        }
        the_title(); ?></h2>
    <div><img localsrc="68" alt=""/><font
            color="<?php echo $ks_settings['author_color']; ?>"><?php the_author(); ?></font>
        <img localsrc="46" alt=" @ "/><font
            color="<?php echo $ks_settings['author_color']; ?>"><?php the_time(); ?></font></div>
    <?php ks_content( __( '(more...)' ), 0, '', 1000 );
    ks_link_pages(); ?>
    <div><img localsrc="354" alt=""/><font size="-1"><?php echo __( 'Categories' ) . ':';
            ks_category(); ?></font><br/>
        ...
        <dl>
            <?php for( $count = $ks_settings['ol_count']; have_posts(); $count++ ) :
                ...
                <dt><?php
                    ...
                    ?> <img localsrc="46" alt="@ "/><font
                        color="<?php echo $ks_settings['date_color']; ?>"><?php ks_time(); ?></font></dt>
            <?php endfor; ?>
        </dl>
