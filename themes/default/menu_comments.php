...
<?php for( $loop = ks_option( 'ks_separate_comments' ) ? 0 : 1; $loop <= 2; $loop += 2 ){
    if( $loop <= 1 ){
        ?>
            ...
            <?php for( ; $target = array_shift( $comments ); $ol_count++ ) :
                ...
                if( empty( $post->post_password ) ){
					...
					if (! ks_option('ks_separate_comments')) {
                        ...
                        echo esc_url( ks_get_comment_link( $comment ) ); ?>"><font
                                color="<?php echo $ks_settings['date_color']; ?>"><?php ks_comment_datetime();
                                ?></font></a> <img localsrc="<?php comment_type( 68, 112, 112 ); ?>"
                                                   alt="by " /><?php comment_author(); ?>
                        </dd><?php
                    endforeach;
                } else{
                    ?>
