...
if( have_posts() ) :
    ...
    while( have_posts() ) : the_post();
        ?>
        <dt><br/><?php _e( '/-', 'ktai_style' ); ?><font
        color="<?php echo $ks_settings['date_color']; ?>"><?php ks_time(); ?></font>
        <?php
        $icon = '<img localsrc="86" alt="[' . __( 'Comments' ) . '] " />';
        ks_comments_link( array( 'icon' => $icon, 'zero' => '0', 'one' => '1', 'more' => '%', 'none' => '<img localsrc="61" alt="' . __( 'Off', 'ktai_style' ) . '" />', 'sec' => '<img localsrc="120" alt="?" />', 'icon_none' => $icon, 'icon_sec' => $icon, ) );
