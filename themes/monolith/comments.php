...
<dt><a name="comment-<?php comment_ID(); ?>"><?php
    ...
    comment_reply_link( array_merge( array( 'before' => '<div>', 'after' => '</div>', 'reply_text' => '<img localsrc="149" alt="" />' . __( 'Reply' ), 'login_text' => '<img localsrc="120" alt="" />' . __( 'Log in to Reply' ) ), $args, array( 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
    }
...
if( ks_is_comments_list() ) :
    ...
    foreach( $sep_comments as $type => $_comments ) :
        if( isset( $label[$type] ) ){
            echo '<div style="color:white;background-color:teal;font-size:smaller;"><h3>' . $label[$type] . '</h3></div>';
        }
        if( count( $_comments ) ) : ?>
            <dl>
        ...
        <?php endif;
    endforeach;
    ks_comments_post_link( NULL, '<div>', '</div>' ); elseif( ks_is_comment_post() ) :
    ...
    <?php
        ...
        if( is_user_logged_in() ){
            ...
            <?php _e( 'Mail (will not be published)', 'ktai_style' );
            if( $req )
                _e( '(required)' ); ?><br/>
