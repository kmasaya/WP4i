...
<?php if( have_posts() ) :
    ...
    <dl>
        <?php for( $count = 1; have_posts(); $count++ ) : the_post(); ?>
            <dt><br/>
            <div style="<?php echo $ks_settings['title_style']; ?>"><?php
                ks_ordered_link( array( 'count' => $count, 'max' => $max, 'link' => get_permalink(), 'anchor' => '<span style="' . $ks_settings['title_style'] . '">' . get_the_title() . '</span>', 'hide_over_max' => TRUE, ) ); ?></div>
