...
for( $loop = ks_option( 'ks_separate_comments' ) ? 0 : 1; $loop <= 2; $loop += 2 ){
    if( $loop <= 1 ){
        ?>
        <div<?php echo $ks_settings['h2_style']; ?>><h2
                id="comments"><?php _e( 'Recent Comments', 'ktai_style' ); ?></h2></div>
    <?php } else{ ?>
        <br/>
        <div<?php echo $ks_settings['h2_style']; ?>><h2 id="pings"><?php _e( 'Recent Pings', 'ktai_style' ); ?></h2>
        </div>
    <?php
    }
    switch( $loop ){
    ...
    if( $comments ) : ?>
        <dl>
            <?php for( ; $target = array_shift( $comments ); $ol_count++ ) :
                ...
                <dt><br/>
                <div
                    style="<?php echo $ks_settings['title_style']; ?>"><?php ks_ordered_link( array( 'count' => $ol_count, 'max' => $ol_max, 'link' => $link, 'anchor' => '<span style="' . $ks_settings['title_style'] . '">' . get_the_title() . '</span>', 'hide_over_max' => TRUE, ) ); ?></div>
    ...
    <?php else: ?>
        <p><?php $loop <= 1 ? _e( 'No comments yet.' ) : _e( 'No pings yet.', 'ktai_style' );
            ?></p>
    <?php endif; // $comments
} // $loop
ks_footer(); ?>