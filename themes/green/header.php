...
if( !ks_is_redir() ){ // other than redir.php
    $before = '<div style="color:white;background-color:#009933;margin:0;">' . ( ks_is_flat_rate() ? '<h1 style="margin:0"><img src="' . ks_theme_url( KTAI_NOT_ECHO ) . 'icon.gif" alt="" />' /*	'<h1 style="margin:0"><img src="' . ks_theme_url(KTAI_NOT_ECHO) . 'icon-feather.gif" alt="" />' */ /*	'<h1 style="margin:0"><img src="' . ks_theme_url(KTAI_NOT_ECHO) . 'icon-sunflower.gif" alt="" />' */ : '<h1>' );
    ks_page_title( array( 'before' => $before, 'after' => '</h1></div>' ) );
    if( ks_is_front() ){
        ?>
        <p><font size="-1"><?php bloginfo( 'description' ); ?></font></p>
    <?php } ?>
    <div align="right"><font size="-1"><a href="#tail"><img localsrc="30"
                                                            alt="<?php _e( '&darr;', 'ktai_style' ); ?>"/><?php _e( 'Menu', 'ktai_style' ); ?>
            </a></font></div>
