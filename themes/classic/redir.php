...
<head>
    ...
    <title><?php echo ks_redir_has( 'full_url' ) ? __( 'Confirm connecting to external sites', 'ktai_style' ) : __( 'Error linking to external sites', 'ktai_style' ) ?></title>
</head>
<body>
<div><h1 id="logo"><img alt="WordPress"
                        src="<?php echo ks_plugin_url( KTAI_NOT_ECHO ) . KTAI_INCLUDES_DIR; ?>/wplogo.gif"/></h1></div>
<?php if( ks_redir_has( 'full_url' ) ) :
    ...
    <?php
        ...
        <br/><a
            href="<?php echo esc_url( ks_redir_get( 'mobile_url' ) ); ?>"><?php echo htmlspecialchars( ks_redir_get( 'mobile_url' ), ENT_QUOTES ); ?></a>
        <?php if( !ks_redir_same_host() ){ ?>
            <br/><font
                color="red"><?php _e( 'The host is diffrent from the origial. Make sure the valid mobile site.', 'ktai_style' ); ?></font>
        <?php
        }
        if( ks_redir_get( 'mobile_url' ) != ks_redir_get( 'full_url' ) ){
            ?>
    ...
    <p><?php _e( 'If you know that the site is suitable for mobile at the same URL, it is good to jump directly.', 'ktai_style' );
        ?><br/>#.<a
            href="<?php echo esc_url( ks_redir_get( 'full_url' ) ); ?>"><?php echo htmlspecialchars( ks_redir_get( 'full_url' ), ENT_QUOTES ); ?></a>
    </p>
<?php } ?>
    <p><?php _e( 'To copy the URL, use below text field:', 'ktai_style' ); ?></p>
    <form action="">
        <div>*.<input type="text" name="url" size="80" maxlength="255"
                      value="<?php echo esc_url( ks_redir_get( 'full_url' ) ); ?>" accesskey="*"/></div>
    </form>
