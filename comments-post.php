...
try{
    ...
    if( get_option( 'require_name_email' ) && !$user->ID ){
        if( 6 > strlen( $comment_author_email ) || '' == $comment_author )
            throw new Exception( __( 'Error: please fill the required fields (name, email).' ) ); elseif( !is_email( $comment_author_email ) )
    ...
    if( $user->ID ){
        if( isset( $Ktai_Style->admin ) && !$Ktai_Style->ktai->get( 'cookie_available' ) && !preg_match( '!' . preg_quote( dirname( __FILE__ ) . '/' . KtaiStyle::ADMIN_DIR, '!' ) . '!', @$_POST['redirect_to'] ) ){
