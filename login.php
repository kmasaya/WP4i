...
class KtaiStyle_Login{
    ...
    public function __construct( $base ){
        ...
        $this->errors = array( 'oldsession' => __( '<strong>ERROR</strong>: Your login session has been expired. Please login again.', 'ktai_style' ) );
    ...
    private function login(){
        ...
        ?>
        <form method="post" action="./<?php echo esc_attr( basename( __FILE__ ) ); ?>">
            <div>
                <?php _e( 'Username' ) ?><br/><input type="text" name="log" id="user_login" class="input" size="20"
                                                     tabindex="10" istyle="3" mode="alphabet"
                                                     value="<?php echo esc_attr( stripslashes( $user_login ) ); ?>" <?php echo $this->style_input; ?>/><br/>
                <?php _e( 'Password' ) ?><br/><input type="password" name="pwd" id="user_pass" class="input" size="20"
                                                     tabindex="20" istyle="3" mode="alphabet"
                                                     value="" <?php echo $this->style_input; ?>/></div>
            <?php // do_action('login_form');
            if( $this->base->get( 'cookie_available' ) ){
                ?>
                <div><label><input name="rememberme" type="checkbox" id="rememberme" value="forever"
                                   tabindex="90"/> <?php _e( 'Remember Me' ); ?></label></div>
            <?php } ?>
            <p><input type="submit" name="wp-submit" value="<?php _e( 'Log in' ); ?>"/>
                <?php if( !empty( $redirect_to ) ){ ?>
                    <input type="hidden" name="redirect_to"
                           value="<?php echo htmlspecialchars( $redirect_to, ENT_QUOTES ); ?>"/>
                <?php
                }
                if( $this->base->get( 'cookie_available' ) ){
                    ?>
        ...
        <p><img localsrc="64" alt="<?php _e( '&larr;', 'ktai_style' ); ?>"/><a
            href="<?php bloginfo( 'url' ); ?>/"><?php printf( __( 'Back to %s', 'ktai_style' ), get_bloginfo( 'title', 'display' ) ); ?></a>
        <?php
